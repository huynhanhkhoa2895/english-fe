...
const ExerciseVocabulary = forwardRef(({vocabularies, results, step, setStep, handleResult, setData}: {
                    ...
                    displayItem && step <= _vocabularies.length - 1 && <>
                        {_vocabularies.map((vocabulary: Vocabulary, index: number) => <Fragment key={vocabulary.id}>
                            {step === index && <div className={'pb-5'}>
                                <ExerciseInputVocabulary key={vocabulary.id} vocabulary={vocabulary}
                                                         handleResult={clickResult}/>
                            </div>}
                        </Fragment>)}
