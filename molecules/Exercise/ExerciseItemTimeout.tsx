...
const ExerciseItemTimeout = ({content, handleResult, setStep, results, step}: {
    content: QuestionTimeoutType,
    step: number,
    handleResult: any,
    setStep: any,
    results: Result[]
                        ...
                        <div className={'w-full grid grid-cols-1 lg:grid-cols-2 gap-5'}>
                            {
                                content.values.map((value: Vocabulary, index: number) => {
                                        return <Button key={index}
                                                       className={'bg-white text-black border border-primary transition-all duration-300 hover:bg-primary hover:text-white'}
                                                       handleClick={() => clickResult({
