            ...
            <ResultAnswer status={isSubmit ? (dataPush['radio_' + content.id] === content.answer.toLowerCase()) : null}>
              <div className={'flex gap-3 w-full'}>
                <FormControl control={control} type={'radio'} label={"True"} valueInput={"true"}
                             name={"radio_" + content.id} required/>
                <FormControl control={control} type={'radio'} label={"False"} valueInput={"false"}
                             name={"radio_" + content.id} required/>
              </div>
            </ResultAnswer>
            {isShowAnswer && <div
                className={twMerge('capitalize text-sm flex-1 absolute right-0', content.answer.toLowerCase() === dataPush['radio_' + content.id] ? 'text-green' : 'text-red')}>{content.answer}</div>}
          </div>
        </div>
      })}
