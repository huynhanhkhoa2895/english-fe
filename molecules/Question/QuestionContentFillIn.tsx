...
const QuestionContentFillIn = withForm(({
    ...
    return <>
        <div className={'my-5 bg-grey flex gap-3 p-3'}>
            {answers.map((answer, index: number) => answer.display &&
                <FillInItemAnswer key={index} value={answer.answer} valueChoose={chooseAnswer}
                                  onChoose={() => handleChoose(answer.answer)}/>)}
        </div>

        <div className={'flex gap-5 flex-col w-full'}>

            {questions && questions.map((question: any, index: number) => <div
                className={'flex items-center justify-between w-full relative'} key={index}>
                <div className={'w-full'}>{question}</div>
                <ResultAnswer className={'w-full flex justify-around items-center'}
                              classNameChildren={'w-full max-w-[150px] xl:max-w-[250px]'}
                              status={isSubmit ? (dataPush['question_' + datas[index].id] === datas[index].answer.toLowerCase()) : null}
                              key={index}>
                    <>
                        <FillInItemInput isSubmit={isSubmit || false} valueChoose={chooseAnswer}
                                         name={'question_' + datas[index].id} control={control}
                                         onChoose={handleQuestionChoose}/>
                        {isShowAnswer && <div
