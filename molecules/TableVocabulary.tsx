        ...
        <>
            {
                !isFirst && <table className={'table-fixed w-full border-collapse border border-slate-400'}>
                    <thead>
                    <tr>
                        <th className={'border border-slate-300 p-3 cursor-pointer lg:w-[200px]'}
                            onClick={() => setMaskVocabularyAll(!maskVocabularyAll)}>Vocabulary
                        </th>
                        <th className={'border border-slate-300 p-3 cursor-pointer '}
                            onClick={() => setMaskTranslateAll(!maskTranslateAll)}>Translate
                        </th>
                        {
                            !isMobile && (
                                <>
                                    <th className={'border border-slate-300 p-3 cursor-pointer '}
                                        onClick={() => setMaskDefineAll(!maskDefineAll)}>Definition
                                    </th>
                                    <th className={'border border-slate-300 p-3 cursor-pointer '}
                                        onClick={() => setExampleDefineAll(!maskExampleAll)}>Example
                                    </th>
                                </>
                            )
                        }
